...
export function ClientDetailsDrawer({ clientId, onClose }: ClientDetailsDrawerProps) {
  ...
  useEffect(() => {
    ...
    Promise.all([getClientWithProjects(clientId), getProjectsByClient(clientId)])
      .then(([clientResult, projectsResult]) => {
        if (clientResult.data) {
          setClient(clientResult.data as ClientData)
        }
  ...
  const displayName = client.primary_contact_name ?? client.name
  ...
  // Shared files placeholder - in production, these would come from Supabase
  ...
  return (
    <div className="fixed inset-0 z-40 flex justify-end">
      ...
      <aside className="flex h-full w-full max-w-2xl flex-col border-l border-border bg-background shadow-2xl">
          ...
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              <div className="hidden sm:block">
                <ClientStatusFilterPopover clientId={client.id} initialStatus={client.status} />
              </div>
              <Button
                variant="ghost"
                size="icon-sm"
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              {client.project_count > 0 && (
                <p className="text-[11px] text-muted-foreground">{client.project_count} linked</p>
              )}
            </div>
            {projects.length === 0 ? (
            ...
            {sharedOpen && (
              <div className="grid gap-3 sm:grid-cols-2">
                {sharedFiles.map((file) => (
                  <div key={file.id} className="rounded-2xl border border-border bg-card/80 px-3 py-2">
        ...
        <div className="flex items-center justify-between bg-background px-5 py-3 border-t border-border">
        ...
        {isWizardOpen && (
          <ClientWizard
            ...
            onSubmit={() => {
              ...
              // Refetch client data
              if (clientId) {
                getClientWithProjects(clientId).then((result) => {
                  if (result.data) {
        ...
        {isProjectWizardOpen && (
          <ProjectWizard
            ...
            onCreate={() => {
              ...
              // Refetch projects
              if (clientId) {
                getProjectsByClient(clientId).then((result) => {
                  if (result.data) {
