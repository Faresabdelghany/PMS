...
export function ActivityTodayCount({ activities }: { activities: ActivityItem[] }) {
  const now = new Date()
  const count = activities.filter((a) => isSameDay(a.created_at, now)).length
  return <span className="rounded-md border bg-muted/40 px-2 py-1">{count} today</span>
}
